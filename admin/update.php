<?php
  ...
  if(isset($_GET['id']) && preg_match("/^[0-9]+$/i",$_GET['id'])){
      ...
      $req = $database->query("SELECT * FROM users WHERE id = $id");
    ...
    if(empty($errors)){
      if(checkCsrf() === true){
        if(isset($pass) && $pass === true){
          $id = $_GET['id'];
          $req = $database->prepare("UPDATE users SET username = ?, password = ?, id_rank = ? WHERE id = ?");
          $password = password_hash($_POST['password'], PASSWORD_BCRYPT);
          $req->execute([$_POST['username'], $password, $_POST['rank'], $id]);
        }else{
          $req = $database->prepare("UPDATE users SET username = ?, id_rank = ? WHERE id = ?");
